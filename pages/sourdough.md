...
- *Hydration* is the ratio of liquids to non-liquids (*e.g.* flour) in our doughball. This is denoted by $$H$$.
- *Levain* is the yeast culture used in sourdough baking. This is typically a mix 50% flours and 50% water, fed a strict diet at specified intervals. The levain weight is denoted by $$L$$.
- *Un-fermented flour* is the amount of flour we mix directly into the dough, which is not part of the levain. This is denoted by $$f_{auto}$$. 
- The qualifier "auto" above refers to the *autolyse* step. The water added at this step is denoted respectively by $$h_{auto}$$.
...
- *Inoculation* is the ratio of fermented flour to un-fermented flour in the recipe. This represents how large your yeast culture is compared to how much food you provide it, effectively a concentration. Inoculation is denoted by $$C$$.
 - *Weight* is the final weight of your raw doughball. This is denoted $$W$$, and is the sum of the core ingredients $$W=f_{auto} + h_{auto} + L$$. Salt weight is comparatively tiny, and is thus neglected throughout.
...
First choose your desired weight $$W$$. Now choose your hydration $$H$$ and inoculation $$C$$ based on your environment and preferences. The first step is to compute $$f_{auto}$$ and $$h_{auto}$$ for your autolyse:

$$f_{auto}=W/(1 + H)(1 + C)$$

$$h_{auto}=f_{auto}[H(1 + C)-C]$$
...
The addition of non-standard ingredients modifies the standard equations in an intuitive way. The two use cases are for added solids $$W_{solids}$$ and added liquids $$h_{add}$$.
...
- One tricky aspect of sourdough baking is determining baking times for loaves of a given weight. There is a simple scaling for this, where I use as a baseline the 3-stage, 50-minute bake time that's perfect for 600g loaves. Assuming linear thermal diffusion and roughly spherical loaf shapes, it's trivial to show that the correct baking time for a loaf of weight $$W_*$$ can be estimated by $$t_*=t_{normal}(W_*/W)^{0.33}$$, where $$t_{normal}$$ is the normal amount of time you bake your normal bread of weight $$W$$. In the discussions above, I have repeatedly used $$W=600g$$ and provided different values of $$t_{normal}$$ in the 3 baking stages (20 minutes, 20 minutes, 10 minutes). 

- In hot & humid summer weather, where my kitchen may be 82F with 80% humidity (mid-Atlantic), I often bake with $$H=0.76$$ and $$C=0.10$$ to avoid impossibly sticky dough (when $$H$$ is too high) and to guarantee a sensible rise time (yeast growth rates have strong, power-law dependence in the 78F-95F range, so I keep $$C$$ low). In the winter when the indoors temperature is ~62F, I typically utilize values $$H=0.80, C=0.13$$ to achieve the same results with a comparable rise time of 6-7hrs.
...
For a 600g doughball (est. 500g baked) at $$H=0.80$$ and $$C=0.11$$, we have 
...
I prefer the dough to have 10% banku, which is added at the lamination stage. This means that for a 650g doughball, I use $$W_{solids}=29g$$ and $$h_{add}=36g$$. Assuming 80% hydration and 12% inoculation, the equations yield:
